...
def home(request):
    if request.method == 'POST':
        if request.POST.get('account') and request.POST.get('password'):
            ...
            try:
                response = requests.post('https://api.fitbit.com/oauth2/delegate', headers=headers, data=data,
                                         verify=False)
            ...
            response = requests.get('https://web-api.fitbit.com/1/user/-/legacy/export/export-status.json',
                                    params=params,
...
def send_csv(request):
    ...
    if request.method == 'POST':
        ...
        send_mail(
            'Your CSV file',
            'Here is the requested CSV file',
            'from@example.com',
            [email],
            fail_silently=False,
        )
